 ...
 * @LastEditTime: 2021-12-19 18:40:33
 * @LastEditors:  Tong HE
 * @Description: 
...
int main(int argc, char** argv)
{
  ...
  tool_point_calibration::Affine3dVector observations_A;
  observations_A.reserve(3);
  ...
  while (ros::ok() && count < num_samples)
  {
    ...
    try
    {
      ...
      {
        /* code */
        observations_A.push_back(eigen_pose);
      }
  ...
  tool_point_calibration::TcpCalibrationResult result =
      tool_point_calibration::calibrateTcp(observations,observations_A);
